...
float getMagnitude(Vec2 vec) { return sqrt(pow(vec.x, 2) + pow(vec.y, 2)); }
...
Vec2 getSubtractedVec(Vec2 a, Vec2 b) { return (Vec2){a.x - b.x, a.y - b.y}; }

Vec2 getAddedVec(Vec2 a, Vec2 b) { return (Vec2){a.x + b.x, a.y + b.y}; }
...
Vec2 getZeroVec2() { return (Vec2){0, 0}; }

Vec3 getZeroVec3() { return (Vec3){0, 0, 0}; }

void ResponseCollisionCircles(Circle c1, Circle c2, Vec2& v1, Vec2& v2) {
  auto m1 = getMagnitude(v1);
  auto m2 = getMagnitude(v2);

  auto p1 = (Vec2){c1.x, c1.y};
  auto p2 = (Vec2){c2.x, c2.y};

  v1 = getScaledVec(getNormalizedVec(getSubtractedVec(p1, p2)), m1);
  v2 = getScaledVec(getNormalizedVec(getSubtractedVec(p2, p1)), m2);
}
